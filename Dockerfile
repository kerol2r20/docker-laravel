...
RUN sed -i 's/deb.debian.org/ftp.tw.debian.org/g' /etc/apt/sources.list && \
    apt-get update && \
    apt-get install -y git libcurl4-openssl-dev libxml2-dev zlib1g-dev && \
    docker-php-source extract && \
    CFLAGS="-I/usr/src/php" docker-php-ext-configure xmlreader && \
    docker-php-ext-install pdo pdo_mysql mysqli curl json xml xmlwriter xmlreader tokenizer mbstring session zip && \
    ...
    useradd -m -r -s /bin/bash laravel && \
